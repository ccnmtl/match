...
def setup_browser():
    ...
    move_to = "window.moveTo(0, 1); window.resizeTo(1024, 768);"
    world.firefox.execute_script(move_to)
...
def i_am_at_the_name_page(step, name):
    if world.using_selenium:
        ...
        try:
            ...
            msg = "Page title is %s. Expected %s" % (title, name)
            assert title.find(name) > -1, msg
        except:
            time.sleep(1)
            title = world.firefox.title
            msg = "Page title is %s. Expected %s" % (title, name)
...
def there_is_no_direction_navigation(step, direction):
    try:
        world.firefox.find_element_by_id(direction)
