...
def there_are_count_topics(step, count):
    try:
        elts = world.browser.find_elements_by_css_selector("div.accordion-group")
...
def the_clock_reads_count_seconds(step, count):
    try:
        elt = world.browser.find_element_by_css_selector("span.display_time_text")
    except:
...
def topic_is_enabled(step, topic):
    ...
    for e in elts:
        ...
        if div.text.find(topic) > -1:
            ...
            enabled = value == "false" or value == None
            assert enabled, "%s is %s. Expected this button to be enabled" % (btn.text, btn.get_attribute('disabled'))
            return
