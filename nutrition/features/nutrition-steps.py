...
def the_patient_chart_contains_text(step, text):
    elt = world.firefox.find_element_by_id("patient-chart-text")
    try:
        assert elt.text.find(text) > -1, "The patient chart reads %s. Expected %s" % (elt.text, text)
    except:
        time.sleep(1)
        assert elt.text.find(text) > -1, "The patient chart reads %s. Expected %s" % (elt.text, text)
...
def all_topics_are_disabled(step):
    elts = world.firefox.find_elements_by_css_selector(".btn.discuss")
    for e in elts:
        assert e.get_attribute('disabled') == "true", "%s is %s. Expected all discussion buttons should be disabled." % (e.text, e.get_attribute('disabled'))
...
def topic_is_disabled(step, topic):
    ...
    for e in elts:
        ...
        if div.text.find(topic) > -1:
            btn = e.find_element_by_css_selector('.btn.discuss')
            assert btn.get_attribute('disabled') == "true", "%s is %s. Expected this button to be enabled" % (btn.text, btn.get_attribute('disabled'))
            return
...
def topic_is_enabled(step, topic):
    ...
    for e in elts:
        ...
        if div.text.find(topic) > -1:
            btn = e.find_element_by_css_selector('.btn.discuss')
            assert btn.get_attribute('disabled') == "false", "%s is %s. Expected this button to be enabled" % (btn.text, btn.get_attribute('disabled'))
            return
...
def i_close_the_discussion(step):
    elts = world.firefox.find_elements_by_css_selector(".btn.complete")
    for e in elts:
